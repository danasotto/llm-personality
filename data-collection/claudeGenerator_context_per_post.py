...
# הגדרת המפתח של Claude
client = anthropic.Anthropic(
    api_key="<add your key>")

df = pd.read_csv("unique_posts_with_long_comment_counts.csv")

print ("drop duplicates = " + str(len(df.drop_duplicates(subset=["post_id"]))))
print( "len: " + str(len(df)))

# GPT settings
temperature = 0.7
model = "claude-sonnet-4-20250514"
output_file = "claude-sonnet-4-0_temp_0.7_generated_comments.csv"
...
for index, row in df.sample(frac=1, random_state=42).iterrows():
    ...
    try:
        ...
        all_comments = response.content[0].text
